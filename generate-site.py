...
def create_repo():
	try:
		repo_name = input("Type the name of the website you are creating (replace whitespaces with hyphens), e.g. for isomerpages/new-repo, type new-repo \n")
...
def create_simple_pages(repo_full_name, nav_yml_path):
	try:
		user_input = input("Type your list of simple-pages that you want to create with spaces, e.g. faq privacy-statement terms-of-use who-we-are \nIf there are no simple pages, please hit enter\n")
		if (user_input != ''):
...
def create_leftnav_pages(repo_full_name, nav_yml_path, config_yml_path):
	try:
		user_input = input("Type your list of leftnav-pages titles and their subpages in the sequence you would like them to be displayed, e.g. left-nav-one:sub-page-A,sub-page-B left-nav-two:sub-page-C,sub-page-D\nIf there are no leftnav pages, please hit enter\n")
		if (user_input != ''):
...
def create_resource_room(repo_full_name, nav_yml_path, config_yml_path):
	try:
		user_input = input("Type the name of your resource room and their subcategories as such, e.g. resource-room:subcategory-A,subcategory-B,subcategory-C\nIf there is no resource room, please hit enter\n")
		if (user_input != ''):
